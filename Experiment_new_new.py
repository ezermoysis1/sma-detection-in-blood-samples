...
class Experiment:
    ...
    def _create_dataloader(self):
        ...
        # Create dataset
        dataset = CustomImageDataset(root_dir=self.new_path, class_to_idx=class_to_idx, min_number_images=self.min_number_images, transform=self.transform)
        ...
        train_indices, test_indices, _, _ = train_test_split(list(range(len(dataset))), labels, test_size=test_size, stratify=labels, random_state=self.seed)
        ...
        if self.oversampling == 1: # Resampling
            ...
            train_indices_resampled, _ = ros.fit_resample(np.array(train_indices).reshape(-1, 1), [labels[i] for i in train_indices])
            ...
            train_indices_resampled, _ = smote.fit_resample(np.array(train_indices).reshape(-1, 1), [labels[i] for i in train_indices])
        ...
        if self.oversampling == 3: # Adasyn
            ...
            # Apply the transform to the training set
            train_indices_resampled, _ = ada.fit_resample(np.array(train_indices).reshape(-1, 1), [labels[i] for i in train_indices])
            ...
            # Create data loaders for each split
            train_loader = DataLoader(torch.utils.data.Subset(dataset, train_indices), batch_size=self.batch_size, shuffle=True)
            test_loader = DataLoader(torch.utils.data.Subset(dataset, test_indices), batch_size=self.batch_size, shuffle=False)
    ...
    def train(self, epochs, bag_size, class_weight):
        ...
        for epoch in range(epochs):  # loop over the dataset multiple times
            ...
            with torch.no_grad():
                for data in self.test_loader:
                    ...
                    if epoch == epochs-1:
                        ...
                        diagnosis = self.df_samples[self.df_samples['FASt-Mal-Code'] == id]['Diagnosis'].values[0]
            ...
            test_pr_auc = average_precision_score(test_act_labels, test_pred_probs)
    ...
    def plot_metrics(self):
        ...
        plt.plot(self.train_accuracy_list, color='red', label='Training accuracy (%)')
        plt.plot(self.test_accuracy_list, color='blue', label='Test accuracy (%)')
        plt.plot(self.test_f1_tracker, color='green', label='Test F1 score (%)')
        plt.plot(self.test_roc_auc_tracker, color='orange', label='Test ROC AUC (%)')
        plt.plot(self.test_pr_auc_tracker, color='pink', label='Test PR AUC (%)')
    ...
    def get_loader_class_distribution(self):
        ...
        for loader, name in zip(loaders, loader_names):
            ...
            distributions[name] = {"total": total, "class_counts": class_counts}

        return distributions
    
    
    def get_loader_diagnosis_distribution(self):
        self.df_samples = pd.read_csv('samples_dataset.csv')  # load the dataframe
        ...
        for loader, name in zip(loaders, loader_names):
            ...
            diagnosis_counts = self.df_samples[self.df_samples['FASt-Mal-Code'].isin(subfolder_names)]['Diagnosis'].value_counts().to_dict()
    ...
    def plot_distribution(self, distributions):
        ...
        for i, (name, diagnosis_counts) in enumerate(distributions.items()):
            df = pd.DataFrame(list(diagnosis_counts.items()), columns=['diagnosis', 'count'])
            ...
            axs[i].tick_params(axis='x', rotation=45)  # Rotate labels on the x-axis for better visibility

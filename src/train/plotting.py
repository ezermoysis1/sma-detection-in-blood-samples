...
def plot_metrics_separately(train_accuracy_list_tracker, test_accuracy_list_tracker, 
    ...
    legend_patches = [mpatches.Patch(color=color, label=diagnosis) for diagnosis, color in color_dict.items()]

    for i in range(len(train_accuracy_list_tracker)):
        axes[0][i].plot(train_accuracy_list_tracker[counter], color='red', label='Training accuracy (%)')
        axes[0][i].plot(test_accuracy_list_tracker[counter], color='blue', label='Test accuracy (%)')
        axes[0][i].plot(test_f1_tracker_list[counter], color='green', label='Test F1 score (%)')
        axes[0][i].plot(test_roc_auc_tracker_list[counter], color='orange', label='Test ROC AUC (%)')
        axes[0][i].plot(test_pr_auc_tracker_list[counter], color='pink', label='Test PR AUC (%)')
        axes[0][i].legend()
        ...
        df_train = pd.DataFrame(list(distributions_train.items()), columns=['diagnosis', 'count'])
        sns.barplot(data=df_train, x='diagnosis', y='count', ax=axes[2][i], palette=color_dict, order=color_dict.keys())
        axes[2][i].legend(handles=legend_patches)
        axes[2][i].set_title(f'Counts of each diagnosis (Train)')
        axes[2][i].set_ylabel('Count')
        axes[2][i].set_xticklabels(list(color_dict.keys()), rotation=45, ha='right')  # Use all diagnoses as x-axis labels
        ...
        distributions_test = {k: distributions_test[k] for k in color_dict.keys()}
        correct_distributions = {k: correct_distributions[k] for k in color_dict.keys()}
        
        df_test = pd.DataFrame(list(distributions_test.items()), columns=['diagnosis', 'count'])
        df_correct = pd.DataFrame(list(correct_distributions.items()), columns=['diagnosis', 'correct_count'])

        df_merged = df_test.merge(df_correct, on='diagnosis')
        
        barplot1 = sns.barplot(data=df_merged, x='diagnosis', y='count', ax=axes[3][i], palette=color_dict, order=color_dict.keys())
        barplot2 = sns.barplot(data=df_merged, x='diagnosis', y='correct_count', ax=axes[3][i], palette=color_dict, hatch='//', order=color_dict.keys())
        ...
        axes[3][i].set_xticklabels(list(color_dict.keys()), rotation=45, ha='right')  # Use all diagnoses as x-axis labels
...
def plot_metrics_together(train_accuracy_list_tracker, test_accuracy_list_tracker,test_f1_tracker_list):
    ...
    ax.plot(mean_train_accuracy, color='red', label='Mean Training accuracy (%)')
    ...
    ax.fill_between(range(len(mean_train_accuracy)), mean_train_accuracy-std_train_accuracy, mean_train_accuracy+std_train_accuracy, color='red', alpha=0.2)
    ax.fill_between(range(len(mean_test_accuracy)), mean_test_accuracy-std_test_accuracy, mean_test_accuracy+std_test_accuracy, color='blue', alpha=0.2)
    ax.fill_between(range(len(mean_test_f1)), mean_test_f1-std_test_f1, mean_test_f1+std_test_f1, color='green', alpha=0.2)
...
def filter_and_plot(directory, csv_file):
    ...
    dirs_to_check = [os.path.join(directory, 'sma'), os.path.join(directory, 'non-sma')]

    # Get the names of all subdirectories in 'sma' and 'non-sma'
    subdir_names = [d for dir_to_check in dirs_to_check for d in os.listdir(dir_to_check) if os.path.isdir(os.path.join(dir_to_check, d))]
    ...
    not_in_df = [subdir_name for subdir_name in subdir_names if subdir_name not in fast_mal_code_set]
    ...
    for subdir_name in not_in_df:
        ...
        if modified_name in fast_mal_code_set:
            ...
            print(f"Neither '{subdir_name}' nor '{modified_name}' are in the csv file")
    ...
    df_samples['Category'] = df_samples['Diagnosis'].apply(lambda x: 'SMA' if x.lower() == 'severe malaria anaemia' else 'Non-SMA')

    # Pivot table for category and diagnosis
    df_pivot = df_samples.pivot_table(index='Category', columns='Diagnosis', aggfunc='size', fill_value=0)
    ...
    # Plot
    ax = df_pivot.plot(kind='bar', stacked=True, color=color_list, figsize=(4,8), edgecolor='black')
    ...
    for i, diagnosis in enumerate(df_pivot.columns):
        ...
        for j, count in enumerate(counts):
            if count > 0:
                plt.text(j, bottoms[j] + count/2, str(count), color='white', ha='center', va='center', weight='bold')
    ...
    for i in range(df_pivot.shape[0]):
        ...
        plt.text(i, total_count, f"{total_count} samples ({percentage:.0f}%)", ha='center', va='bottom', weight='bold')
    ...
    plt.legend(title='Classes', loc='lower center')  # Set legend title to 'Classes' and position it at the top right
